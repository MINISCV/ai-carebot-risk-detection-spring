...
이 문서는 **고독사 예방을 위한 시니어케어 돌봄로봇 데이터 분석**에서 제공하는 REST API의 명세에 대해 기술합니다. 본 API는 **회원, 인형, 시니어 관리 및 대화 데이터 분석** 기능을 제공하며, 개발자들이 애플리케이션과 서비스를 쉽게 연동할 수 있도록 돕습니다.
...
본 API는 로그인을 제외한 모든 요청에 대해 인증이 필요합니다. 인증 방식으로는 `JWT Bearer Token`을 사용합니다.
...
*   **Method:** `POST`
*   **URL:** `/seniors`
*   **Description:** 새로운 시니어 정보를 생성하고, 사용 가능한 인형을 할당합니다.
*   **인증:** 필수
...
| `guardian_phone`|`string` | Y | 보호자 연락처 |
...
---
#### **4.2. `GET /seniors` - 전체 시니어 목록 조회**
*   **Method:** `GET`
*   **URL:** `/seniors`
*   **Description:** 시스템에 등록된 모든 시니어의 목록을 조회합니다.
*   **인증:** 필수
*   **Success Response (`200 OK`):**
    *   시니어 정보 객체의 배열을 반환합니다.
---
#### **4.3. `GET /seniors/{id}` - 특정 시니어 정보 조회**
*   **Method:** `GET`
*   **URL:** `/seniors/{id}`
*   **Description:** ID로 특정 시니어의 정보를 조회합니다.
*   **인증:** 필수
*   **Path Parameters:**
...
*   **Method:** `PUT`
*   **URL:** `/seniors/{id}`
*   **Description:** ID로 특정 시니어의 정보를 수정합니다.
*   **인증:** 필수
*   **Path Parameters:**
...
*   **Error Responses:**
    ...
    *   `404 Not Found`: CSV에 포함된 인형 ID가 시스템에 등록되어 있지 않은 경우 발생합니다.
    *   `500 Internal Server Error`: 외부 분석 서버와의 통신 오류 등 분석 과정에서 서버 오류가 발생한 경우 반환됩니다.

---
#### **5.2. `GET /analyze` - 전체 분석 결과 목록 조회**

지금까지 분석된 모든 대화의 종합 결과 목록을 조회합니다.

*   **Method:** `GET`
*   **URL:** `/analyze`
*   **Description:** 전체 분석 종합 결과 목록을 조회합니다.
*   **인증:** 필수

*   **Success Response (`200 OK`):**
    *   분석 종합 결과 객체의 배열을 반환합니다.
```json
[
    {
        "id": 1,
...
```
---
#### **5.3. `GET /analyze/{id}` - 특정 분석 결과 조회**

특정 ID를 가진 분석 종합 결과를 조회합니다.

*   **Method:** `GET`
*   **URL:** `/analyze/{id}`
*   **Description:** ID로 특정 분석 종합 결과를 조회합니다.
*   **인증:** 필수
...
*   **Success Response (`200 OK`):**
    *   조회된 분석 종합 결과 정보를 반환합니다. (`5.2`의 응답 객체와 동일)
...
| 토큰 갱신 | `POST` | `/refresh` | 불필요 | Cookie: `refresh_token` | Header: `Authorization` |
| **회원 (Members)** | | | | | |
| 회원 가입 | `POST` | `/members` | 불필요 | Body: `username`, `password` | 생성된 `Member` 객체 |
| 전체 회원 조회 | `GET` | `/members` | 필수 | - | `Member` 객체 배열 |
| 특정 회원 조회 | `GET` | `/members/{username}` | 필수 | Path: `username` | `Member` 객체 |
| 회원 정보 수정 | `PUT` | `/members/{username}` | 필수 | Path: `username`, Body: `role`, `enabled` | 수정된 `Member` 객체 |
| 회원 삭제 | `DELETE` | `/members/{username}` | 필수 | Path: `username` | `204 No Content` |
| **인형 (Dolls)** | | | | | |
| 인형 생성 | `POST` | `/dolls` | 필수 | Body: `id` | 생성된 `Doll` 객체 |
| 전체 인형 조회 | `GET` | `/dolls` | 필수 | - | `Doll` 객체 배열 |
| 특정 인형 조회 | `GET` | `/dolls/{id}` | 필수 | Path: `id` | `Doll` 객체 |
| 인형 삭제 | `DELETE` | `/dolls/{id}` | 필수 | Path: `id` | `204 No Content` |
| **시니어 (Seniors)** | | | | | |
| 시니어 등록 | `POST` | `/seniors` | 필수 | Body: `Senior` 정보 | 생성된 `Senior` 객체 |
| 전체 시니어 조회 | `GET` | `/seniors` | 필수 | - | `Senior` 객체 배열 |
| 특정 시니어 조회 | `GET` | `/seniors/{id}` | 필수 | Path: `id` | `Senior` 객체 |
| 시니어 정보 수정 | `PUT` | `/seniors/{id}` | 필수 | Path: `id`, Body: `Senior` 정보 | 수정된 `Senior` 객체 |
| 시니어 삭제 | `DELETE` | `/seniors/{id}` | 필수 | Path: `id` | `204 No Content` |
| **분석 (Analyze)** | | | | | |
| 대화 파일 분석 | `POST` | `/analyze` | 필수 | Form-data: `file` | `AnalysisResult` 객체 |
| 전체 분석 결과 조회 | `GET` | `/analyze` | 필수 | - | `OverallResult` 객체 배열 |
| 특정 분석 결과 조회 | `GET` | `/analyze/{id}` | 필수 | Path: `id` | `OverallResult` 객체 |
| 분석 결과 삭제 | `DELETE` | `/analyze/{id}` | 필수 | Path: `id` | `204 No Content` |
